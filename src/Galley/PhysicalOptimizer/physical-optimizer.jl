...
function reorder_input(input, expr, loop_order::Vector{IndexExpr})
    ...
    reorder_query = Query(
        Alias(galley_gensym("A")),
        Materialize(formats..., fixed_order..., agg_expr),
        reverse(input_order)...,
    )
    ...
    expr = Rewrite(Postwalk(@rule ~n => final_alias_expr where {n.node_id==input.node_id}))(
        expr
    )
    return [reorder_query], expr
end

function split_plan_to_physical_plan(
    split_plan::PlanNode,
    ST,
    alias_to_loop_order,
    ...
    for s_query in split_plan.queries
        p_queries = logical_query_to_physical_queries(
            s_query, ST, alias_stats; only_add_loop_order=false, transpose_aliases=false
        )
        for p_query in p_queries
            alias_stats[p_query.name.name] = p_query.expr.stats
            for n in PostOrderDFS(p_query.expr)
                if n.kind == Alias
                    alias_to_loop_order[n.name] = IndexExpr[
                        idx.name for idx in p_query.loop_order
                    ]
                end
            end
        end
...
function logical_query_to_physical_queries(
    ...
    if !(@capture query Query(~name, Materialize(~args...))) &&
        ...
        throw(
            ErrorException(
                "Physical optimizer only takes in properly formatted single queries."
            ),
        )
    end
    ...
    loop_order = get_join_loop_order(
        disjunct_and_conjunct_stats,
        collect(values(transposable_stats)),
        output_stats,
        output_order,
    )
    queries = []
    for (id, stats) in transposable_stats
        if !only_add_loop_order &&
            !is_sorted_wrt_index_order(get_index_order(stats), loop_order; loop_order=true)
    ...
    if !isnothing(output_order)
        ...
        if !isnothing(output_formats) && first_formats != output_formats
            intermediate_query = Query(
                Alias(galley_gensym("A")),
                Materialize(first_formats..., output_order..., expr),
                loop_order...,
            )
            ...
            best_formats = if !isnothing(output_formats)
                output_formats
            else
                select_output_format(output_stats, reverse(output_order), output_order)
            end
            ...
            result_query = Query(
                query.name,
                Materialize(best_formats..., output_order..., result_expr),
            ...
            output_formats = if isnothing(output_formats)
                select_output_format(output_stats, loop_order, output_order)
            else
                output_formats
            end
            result_query = Query(
                query.name,
                Materialize(output_formats..., output_order..., expr),
                loop_order...,
            )
            reorder_stats = copy_stats(expr.stats)
            reorder_def = get_def(reorder_stats)
