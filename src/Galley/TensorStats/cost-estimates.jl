...
function get_prefix_cost(
    ...
    rel_disjuncts = [
        stat for stat in disjunct_stats if !isempty(get_index_set(stat) ∩ prefix_set)
    ]
    ...
    for stat in union(rel_conjuncts, rel_disjuncts)
        ...
        if isnothing(get_index_order(stat)) || needs_reformat(stat, new_prefix)
            ...
            approx_sparsity =
                estimate_nnz(
                    stat; indices=rel_vars, conditional_indices=setdiff(rel_vars, [new_var])
                ) / get_dim_size(stat, new_var)
    ...
    if output_vars isa Vector && new_var ∈ output_vars
        ...
        min_var_idx = minimum([
            x for x in indexin(new_prefix, output_vars) if !isnothing(x)
        ])
