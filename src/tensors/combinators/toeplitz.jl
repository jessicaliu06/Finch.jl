...
unwrap_outer(ctx, arr::VirtualToeplitzArray, mode, protos) =
    VirtualToeplitzArray(unwrap_outer(ctx, arr.body, mode, [protos[1:arr.dim]; protos[arr.dim + 2:end]]), arr.dim)
