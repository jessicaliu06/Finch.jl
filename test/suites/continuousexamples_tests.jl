@testitem "continuous_examples" begin
    ...
    @testset "openclosed + openclosed" begin
        ...
        A_val = [1, 2, 3]
        A = Tensor(
            SparseRunList{Float64}(
                Element(0, A_val),
                12.0,
                [1, 4],
                Finch.PlusEpsVector(A_left),
                A_right,
                Element(0, Int[]),
            ),
        )
        ...
        C_ref = Tensor(
            SparseRunList{Float64}(
                Element(0, [1, 2, 1, 3, 1, 2, 3]),
                12.0,
                [1, 8],
                Finch.PlusEpsVector([1.1, 2.2, 5.5, 6.6, 7.7, 8.8, 9.9]),
                [2.2, 3.3, 6.6, 7.7, 8.8, 9.9, 11.11],
                Element(0, Int[]),
            ),
        )
    ...
    @testset "2D Box Search" begin
        ...
        points = Tensor(
            SparseList{Float64}(
                SparseList{Float64}(SparsePoint{Int64}(Pattern(),
                        shape_id,
                        point_id),
                    shape,
                    point_ptr_y,
        ...
        box = Tensor(
            SparseInterval{Float64}(
                SparseInterval{Float64}(Pattern(),
                    shape,
                    box_y_start,
                    box_y_stop),
                shape,
        ...
        def = @finch_kernel mode = :fast function rangequery(output, box, points)
            output .= false
            for x in _, y in _, id in _
                output[id] |= box[y, x] && points[id, y, x]
            end
        end

        radius = ox = oy = 0.0 #placeholder
        def2 = @finch_kernel mode = :fast function radiusquery(
        ...
        for i in 1:length(query)
            ...
            radius =
                sqrt((query[i][1] - query[i][3])^2 + (query[i][2] - query[i][4])^2) / 2.0
    ...
    @testset "Trilinear Interpolation on Sampled Ray" begin
        ...
        grid = Tensor(
            SparseRunList{Float64}(
                SparseRunList{Float64}(SparseRunList{Float64}(Dense(Element(0))))
            ),
            16,
            16,
            16,
